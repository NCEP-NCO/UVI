...
C  Read Albedo field
C  Mask Albedo field for just snow/ice by retaining alb>30% and Tsfc<275 K
C  Adjust field for Albedo (Snow)
C  Read UVBC field
C  Read UVB0 field
C  Determine UV Transmissivity
C
C   OUTPUT FILES:
...
C   MACHINE:  AIX
C
C    CHANGE: Feb 25, 2009 - Changed IFCST=24
C                             to    IFCST=FCSTHR
C
C$$$
...
C
     ...
     > FCSTHR, YY
C
      REAL OZONE(igrd,jgrd), SZA(igrd,jgrd), GLAT, UV(NXY),
     > EDRLUT(18,27), ELEV(igrd,jgrd), ALBEDO(igrd,jgrd), 
     > UVBCLD(igrd,jgrd), UVBCLR(igrd,jgrd), EL(igrd,jgrd),
     > ELADJ(igrd,jgrd), TSFC(igrd,jgrd), ALBSNO(igrd,jgrd)
C
...
C
      OPEN(20, STATUS='OLD', FORM='FORMATTED')
      READ(20,*) nx0, ny0
      READ(20,*) OZONE
      CLOSE(20)
...
C
      ...
      READ(21,*) ALBEDO
      CLOSE(21)
      WRITE(6,FMT='("Albedo Data Read",i4,i4)') nx1,ny1
C
      OPEN(22, STATUS='OLD', FORM='FORMATTED')
      READ(22,*) nx2, ny2 
      READ(22,*) UVBCLR
      CLOSE(22)
      WRITE(6,FMT='("UVBCLR Data Read",i4,i4)') nx2,ny2
C
      OPEN(23, STATUS='OLD', FORM='FORMATTED')
      READ(23,*) nx3, ny3
      READ(23,*) UVBCLD
      CLOSE(23)
      WRITE(6,FMT='("UVBCLD Data Read",i4,i4)') nx3,ny3
C
...
C
      ...
      A3 =  0.00002
C
C...Check surface temperature to be less than 274 K
C...Then use albedo 
C...Still does not exclude cold bright deserts
C
      DO 80 LTT = 1, JGRD
         DO 79 LNN = 1, IGRD
            IF (TSFC(LNN,LTT) .le. 274.0 ) THEN
               ALBSNO(LNN,LTT) = ALBEDO(LNN,LTT)
            ELSE
               ALBSNO(LNN,LTT) = 0.0
            ENDIF
C
...
c.     > MINVAL(ERY5(1:igrd,j))
   90 CONTINUE
C
C...changed from ery1 to ery2 August 25
C
...
C     IYY, IMM, IDD ARE PROVIDED BELOW
      ...
      IDPARA=206
      IDLEVEL=1
C...MODEL ID= 2, UV POTENTIAL INDEX
C...PARAMETER ID=206, UV POTENTIAL INDEX (W/M**2)
